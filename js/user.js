async function createWallet() {
  const res = await fetch(`https://localhost:4433/wallet/create`, { method: 'POST' });
...
async function getBalance() {
  ...
  const res = await fetch(`https://localhost:4433/wallet/balance/${address}`);
  const data = await res.json();
...
async function sendTransaction() {
  ...
  const res = await fetch(`https://localhost:4433/transaction/send`, {
    method: 'POST',
